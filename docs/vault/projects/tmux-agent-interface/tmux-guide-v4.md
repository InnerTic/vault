...
> **Version target:** tmux 3.5+ (user's system: 3.5a). Notes cover 3.7 features where available. Check `tmux -V` for your version.
...
Every entity has a stable ID that never changes — use in scripts:
- **Session:** `$N` (e.g. `$0`)
- **Window:** `@N` (e.g. `@0`)
- **Pane:** `%N` (e.g. `%0`)
...
### Walkthrough A: Basic Session Lifecycle
...
# Kill a session (or use prefix + W, then y)
tmux kill-session -t myproject
```

### Walkthrough B: Multi-Pane Dev Workspace

```bash
# Start a session with a specific directory
tmux new-session -d -s dev -c ~/projects/myapp

# Split into two panes (horizontal)
tmux split-window -v -t dev:0

# Split again (vertical)
tmux split-window -h -t dev:0.1

# Send commands to specific panes
tmux send-keys -t dev:0.0 'npm run dev' Enter      # terminal pane 0
tmux send-keys -t dev:0.1 'npm test' Enter           # terminal pane 1
tmux send-keys -t dev:0.2 'tail -f logs/app.log' Enter  # terminal pane 2
...
### Walkthrough C: Copy-Paste Workflow (Vi Keys)
...
# With clipboard integration, copy also sends to system clipboard:
# prefix + ] pastes from system clipboard (set-clipboard on)
```

### Walkthrough D: Quick Pane Navigation
...
# Better splits with C-b | (vertical) and C-b - (horizontal)
bind | split-window -h
bind - split-window -v
...
Control mode is a text-only protocol between tmux and external programs. It's the most powerful interface for scripting and AI agent integration.
...
The `-CC` flag sends a DSC sequence that listening terminals detect. An empty line (Enter) detaches.
...
Control mode clients receive these when events occur:
...
| Pane output | `%output %id content` |
| Client active | `%client-active` |
| Client detached | `%client-detached` |

### Example: Control Mode Script
...
# Capture pane output
echo -e "capture-pane -t %0 -p" \
    | tmux -Ldefault -C attach-session
```

### Format Subscriptions (3.5+)

Subscribe to any format variable and get notified on change (once/second max). Useful for monitoring GPU usage, build progress, or agent status:
...
Check support: `#{bracket_paste_flag}` format variable.

### OSC 9;4 Progress Bar (3.7)

tmux handles the `ESC]9;4;...ESC\` OSC sequence (iTerm2 / Windows Terminal inline progress). Available as `#{pane_progress}` format.

### Floating Panes (3.7)

Floating panes sit above tiled layout like popups but are non-modal — they behave like real panes with full escape sequence support.

```tmux
# Create floating pane (bound to * by default in 3.7)
bind * new-pane

# Move/resize with mouse; second status line shows all panes:
set -g status-format 2
```

### `remain-on-exit` (3.6+)

Keeps the pane alive after exit until a key is pressed. New value `key` added in 3.7 (previously only `on` and `fail`):
...
`{last}`, `{next}`, `{previous}`, `{top}`, `{bottom}`, `{left}`, `{right}`, `{top-left}`, `{bottom-right}`, `{up-of}`, `{down-of}`, etc.
...
| Project | Description |
|---------|-------------|
| [tmuxp](https://github.com/tmuxp/tmuxp) | Session-as-code YAML layouts. Define reproducible AI workspaces. |
...
| [jawn-tmux](https://github.com/jamditis/jawn-tmux) | tmux session manager for AI agent workflows — visual pane attention, live sidebar. |
...
*Updated: 2026-06-22*
*Source: tmux/tmux.wiki, tmux CHANGES (master), online wiki pages, ecosystem projects*
