# Projects

[[hat-assistant]] project documentation outside the structured vault categories.

## Archived / Abandoned

| Project | Reason |
|---------|--------|
| [[meta-scripts\|Meta Script Project]] | Overengineered — individual scripts (lxc-bootstrap, llama-loader modular) already achieved ad-hoc modularity. No code was ever written. |

| Project | Source | Description |
|---------|--------|-------------|
| [[translation-pipeline\|Translation Pipeline v2.0]] | `/mnt/data/_translation-pipeline/translation-pipeline/` | Multi-stage LLM pipeline — 10 stages (parser→continuity) with per-role model docs in [[translation-pipeline/roles/INDEX\|roles/INDEX]] |
| [[sd-webui-forge-neo\|SD WebUI Forge Neo]] | `/mnt/workspace/sd-webui-forge-neo/` | Stable Diffusion Forge — 3060 (primary) + P40 (VAE/upscale) |
| [[ai-bubble-watchboard\|AI Bubble Watchboard]] | — | Monthly monitoring — infra, enterprise adoption, open models, wrapper death watch |
| [[local-ai-strategy\|Local AI Strategy]] | — | Three-layer architecture + 90-day build plan for local AI |
| [[local-ai-control-tower\|Local AI Control Tower]] | — | Concrete v1 architecture spec — five-plane stack, workflows, build phases, configuration |
| [[ai-watchtower\|AI Watchtower LXC]] | — | Proxmox LXC for market monitoring, source collection, claim extraction, weekly report generation into vault — bootstrap plan included |

