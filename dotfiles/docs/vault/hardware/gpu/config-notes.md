...
**Drift assessment (2026-06-23):** CachyOS boot. P40 may be VFIO-bound depending on current boot; verify `nvidia-smi` before assuming dual-GPU layout.
...
**STALE** — P40 may be VFIO-bound (not visible to nvidia-smi). Model names likely outdated. Verify `nvidia-smi` before relying on this layout.
...
## Server Layout — STALE (P40 VFIO makes dual-server layout inactive on current boot)
