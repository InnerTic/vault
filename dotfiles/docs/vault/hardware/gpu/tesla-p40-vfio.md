...
```bash
echo "ken ALL=(ALL) NOPASSWD: /home/ken/dotfiles/scripts/toggle-p40.sh" | sudo tee /etc/sudoers.d/toggle-p40
```
...
- **Root cause:** The `limine-entry-tool` hook detects [[changelog]] to `/etc/default/limine` and asks for confirmation.
...
### Lesson
Real-world beta testing caught every one of these [[bugs-and-workarounds]]. The script was functionally correct on paper, but edge cases (empty directories, interactive prompts, IFS behavior, pipefail semantics) only surfaced when actually running it against the live system.
